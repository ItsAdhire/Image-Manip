...
/**
 * Represents a set of tests for the LumaGreyScale class.
 */
public class LumaGreyScaleTest {
  ...
  public void testInvalidConstructors() {
    try {
      ImageOp i = new LumaGreyScale("test", null);
    ...
    try {
      ImageOp i = new LumaGreyScale(null, "anothertest");
      fail("Succeeded on null input");
    } catch (IllegalArgumentException e) {
      assertEquals("Given a null argument", e.getMessage());
    }
    ...
    try {
      ImageOp i = new LumaGreyScale(null, null);
      fail("Succeeded on null input");
    } catch (IllegalArgumentException e) {
      assertEquals("Given a null argument", e.getMessage());
    }
    ...
    try {
      ImageOp i = new LumaGreyScale(null);
      fail("Succeeded on null input");
    } catch (IllegalArgumentException e) {
      assertEquals("Given a null argument", e.getMessage());
    }
  ...
  public void testOperation() {
    new LumaGreyScale("single").execute(im0);
    new LumaGreyScale("wide", "wide-changed").execute(im1);
    new LumaGreyScale("tall", "tall-changed").execute(im2);
    new LumaGreyScale("square").execute(im3);
    new LumaGreyScale("single", "single-changed").execute(im3);

    int[] out0 = new int[]{0x494949};
    int[] out1 = new int[]{0x000000, 0xFFFFFFFF, 0x1E1E1E};
    int[] out2 = new int[]{0x000000, 0xFFFFFFFF, 0x1E1E1E};
    int[] out3 = new int[]{0x010101, 0x020202, 0x333333, 0xEDEDED};
  ...
  public void testNoImageFound() {
    try {
      new LumaGreyScale("single").execute(im1);
    ...
    try {
      new LumaGreyScale("not found", "doesnt matter").execute(im2);
      fail("Succeeded when no image with that name");
    } catch (IllegalStateException e) {
    ...
    try {
      new LumaGreyScale("single-changed", "single-changed").execute(im3);
      fail("Succeeded when no image with that name");
    } catch (IllegalStateException e) {
    ...
    try {
      new LumaGreyScale("abcdefghijklmop").execute(im0);
      fail("Succeeded when no image with that name");
    } catch (IllegalStateException e) {
