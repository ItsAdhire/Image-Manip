...
/**
 * Represents a set of tests for the VerticalFlip class.
 */
public class VerticalFlipTest {
  ...
  public void testInvalidConstructors() {
    try {
      ImageOp i = new VerticalFlip("test", null);
    ...
    try {
      ImageOp i = new VerticalFlip(null, "anothertest");
      fail("Succeeded on null input");
    } catch (IllegalArgumentException e) {
      assertEquals("Given a null argument", e.getMessage());
    }
    ...
    try {
      ImageOp i = new VerticalFlip(null, null);
      fail("Succeeded on null input");
    } catch (IllegalArgumentException e) {
      assertEquals("Given a null argument", e.getMessage());
    }
    ...
    try {
      ImageOp i = new VerticalFlip(null);
      fail("Succeeded on null input");
    } catch (IllegalArgumentException e) {
      assertEquals("Given a null argument", e.getMessage());
    }
  ...
  public void testOperation() {
    new VerticalFlip("single").execute(im0);
    new VerticalFlip("wide", "wide-changed").execute(im1);
    new VerticalFlip("tall", "tall-changed").execute(im2);
    new VerticalFlip("square").execute(im3);
    new VerticalFlip("single", "single-changed").execute(im3);

    int[] out0 = new int[]{0xFF00FF};
    int[] out1 = new int[]{0x012345, 0xFFFFFFFF, 0x000000};
    int[] out2 = new int[]{0x000000, 0xFFFFFFFF, 0x012345};
    int[] out3 = new int[]{0x020202, 0x010101, 0xFFFF00, 0x333333};
  ...
  public void testNoImageFound() {
    try {
      new VerticalFlip("single").execute(im1);
    ...
    try {
      new VerticalFlip("not found", "doesnt matter").execute(im2);
      fail("Succeeded when no image with that name");
    } catch (IllegalStateException e) {
    ...
    try {
      new VerticalFlip("single-changed", "single-changed").execute(im3);
      fail("Succeeded when no image with that name");
    } catch (IllegalStateException e) {
    ...
    try {
      new VerticalFlip("abcdefghijklmop").execute(im0);
      fail("Succeeded when no image with that name");
    } catch (IllegalStateException e) {
